...
def main():
    ap = argparse.ArgumentParser(
        description="Plain GradientBoosting baseline (no tuning)"
    )
    ...
    ap.add_argument(
        "--results_dir",
        default=r"C:\Projects\U\Pro_fin3\Models\gradient_boosting\raw\results",
    )
    ap.add_argument(
        "--figures_dir",
        default=r"C:\Projects\U\Pro_fin3\Models\gradient_boosting\raw\figures",
    )
    ...
    # Train (PURE DEFAULTS)
    # Default sklearn params: n_estimators=100, learning_rate=0.1, max_depth=3, subsample=1.0, etc.
    model = GradientBoostingClassifier()  # <-- no params set
    ...
    n = len(yte)
    if n <= args.metrics_sample_max:
        idx = np.arange(n)
    else:
        idx = rs.choice(n, size=args.metrics_sample_max, replace=False)
    ...
    if imps is not None:
        ...
        plt.close(fig)
        pd.DataFrame(
            {"feature": [feature_names[i] for i in order], "importance": imps[order]}
    ...
    print(f"[RAW GBC] fit_time={fit_time:.2f}s")
    print(
        f"[RAW GBC] ROC-AUC={metrics['roc_auc']:.6f}, AP={metrics['average_precision']:.6f}"
    )
    if thr_opt is not None:
        print(f"[RAW GBC] thr_opt={thr_opt:.4f}, F1_opt={metrics_opt['f1']:.6f}")
